...
- With large molecules over 100 atoms, [numeric atom types](https://github.com/openforcefield/smirnoff99Frosst/issues/76) written by ParmEd will be truncated by AmberTools. One way to work around this is to rewrite atom types with random characters of length (e.g., `101` → `a9`). There can still be trouble with hardcoded recognition of inferring elements from atom types (see Niel's [`aimtools`](https://github.com/nhenriksen/aimtools) repository for more information).
...
Run the `jupyter notebook`s and choose the `smirnovert` environment for the kernel.
### Assumptions
